...
bptest(mod1)
```
Using the same chi-squared critical value of $11.0705$, we can no longer reject the null hypothesis of homoskedasticity as the BP test value was only $9.0107$. This does not prove we have homoskedasticity, only that we cannot prove we have heteroskedasticity for this model.
...
diag(S)
# consistenly getting the wrong estimates. Not sure why.
# HW_standard_errors = df_adj * solve(t(X)%*%X) %*% (t(X)%*%e_sqrd%*%X) %*%solve(t(X)%*%X)
```