...
(f) Explain the issue with using non-parametric RD like we just did for the past two regressions? 

**Solution:**
Non-parametric RD allows for a trade off in the bias for an increase in variance due to the decrease in observation. However, as you make the bandwidth too large there are not enough observations to make accurate estimates. 

### Question 3

(a) Suppose the Spanish government instead decided to pay all prospective mothers (those trying to have children) a smaller benefit, and those who actually had children a greater benefit on top of what they had all ready been paid. What method would a researcher use to find the causal effect this policy has on estimated conceptions? What could a potential regression look like for this design?
...
Where $log_n$ is the log number of estimated conceptions and $\hat{y_{(i)}}$ represents the estimated log income of prospective mothers and mothers. Obviously this model could suffer from a lot of problems.
...
**Solution:**
As with any other model or design that uses IV and 2SLS there are issues with large standard errors, as well as a potential weak variable.
